...
declare -A IMPLEMENTATIONS=(
    ...
    # ["coll_IO_der2"]="../src/bin/collectiveIO_derData2"
    ["ind_IO+isend"]="../src/bin/independentIO_derData_and_isend"
)
...
DATASETS=(
    ...
    # "../data/art_data_128_128_128_7.bin"
...
# Get output directory name from command line argument, or use timestamp if not provided
if [ "$#" -ge 1 ]; then
    ...
    for processes in "${PROCESS_COUNTS[@]}"; do
        ...
        for impl_name in "${!IMPLEMENTATIONS[@]}"; do
            ...
            for ((i=0; i<ITERATIONS; i++)); do
                ...
                output_file="${RAW_DIR}/output_$(basename "$dataset" .bin)_${processes}.txt"
